...
describe('Item Use-Cases', () => {
  ...
  describe('Create Item', () => {
    it('should create an item successfully', async () => {
      ...
      const item = await useCase.execute({ name: 'Test Item', description: 'A test item' });
    ...
    it('should throw InvalidItemDataException when data is missing', async () => {
      const useCase = new ItemCreateUseCase(repo);
      await expect(useCase.execute({ name: '', description: '' })).rejects.toThrow(InvalidItemDataException);
    });

    it('should throw ItemAlreadyExistsException when name duplicates', async () => {
      const useCase = new ItemCreateUseCase(repo);
      await useCase.execute({ name: 'Unique', description: 'first' });
      await expect(useCase.execute({ name: 'Unique', description: 'second' })).rejects.toThrow(ItemAlreadyExistsException);
    });
  });
  ...
  describe('Update Item', () => {
    it('should update an existing item', async () => {
      ...
      const created = await create.execute({ name: 'ToUpdate', description: 'desc' });

      const update = new ItemUpdateUseCase(repo);
      const updated = await update.execute({ id: created.id, name: 'Updated', description: 'newdesc' });
    ...
    it('should throw InvalidItemDataException when id is missing', async () => {
      ...
      await expect(update.execute({})).rejects.toThrow(InvalidItemDataException);
    });

    it('should throw ItemNotFoundException for non-existing id', async () => {
      const update = new ItemUpdateUseCase(repo);
      await expect(update.execute({ id: 'non-existent', name: 'x' })).rejects.toThrow(ItemNotFoundException);
    });
  });
  ...
  describe('Delete Item', () => {
    it('should delete an existing item', async () => {
      const create = new ItemCreateUseCase(repo);
      const created = await create.execute({ name: 'ToDelete', description: 'desc' });
    ...
    it('should throw InvalidItemDataException when id is missing', async () => {
      const del = new ItemDeleteUseCase(repo);
      // @ts-ignore intentional incorrect payload
      await expect(del.execute({})).rejects.toThrow(InvalidItemDataException);
    });

    it('should throw ItemNotFoundException for non-existing id', async () => {
      const del = new ItemDeleteUseCase(repo);
      await expect(del.execute({ id: 'no-id' })).rejects.toThrow(ItemNotFoundException);
    });
  });
});
